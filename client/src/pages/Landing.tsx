        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            ...
            <div className="bg-[#fafaf8] border border-stone-200 rounded-2xl overflow-hidden">
              <div className="aspect-video bg-slate-900 p-4 flex flex-col">
                {/* Step wizard */}
                <div className="flex items-center justify-center gap-1 mb-4">
                  {['Credentials', 'Review', 'Payment', 'Progress'].map((s, i) => (
                    <div key={s} className="flex items-center gap-1">
                      <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-semibold ${i === 0 ? 'bg-amber-500 text-slate-900' : 'bg-slate-700 text-slate-400'}`}>
                        <span>{i + 1}</span>
                        <span className="hidden sm:inline">{s}</span>
                      </div>
                      {i < 3 && <div className="w-3 h-px bg-slate-600" />}
                    </div>
                  ))}
                </div>
                {/* Two credential panels */}
                <div className="flex gap-2 flex-1">
                  {['Source Account', 'Destination Account'].map((label, idx) => (
                    <div key={label} className="flex-1 bg-slate-800 rounded-lg p-2">
                      <p className="text-[9px] text-slate-400 font-medium mb-1.5">{label}</p>
                      <div className="space-y-1.5">
                        {['Client ID', 'Client Secret'].map((field) => (
                          <div key={field}>
                            <p className="text-[8px] text-slate-500 mb-0.5">{field}</p>
                            <div className="h-4 bg-slate-700 rounded flex items-center px-1.5">
                              <div className={`h-1.5 rounded ${idx === 0 ? 'w-16 bg-slate-500' : 'w-12 bg-slate-500'}`} />
                            </div>
                          </div>
                        ))}
                        <div className={`mt-1.5 h-5 rounded flex items-center justify-center text-[8px] font-semibold ${idx === 0 ? 'bg-emerald-600 text-white' : 'bg-slate-700 text-slate-400'}`}>
                          {idx === 0 ? '✓ Validated' : 'Validate →'}
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div className="p-5">
                <h3 className="text-lg font-bold text-slate-900 mb-1">Connect &amp; Validate</h3>
                <p className="text-slate-500 text-sm leading-relaxed">Enter your source and destination Guesty API credentials. Both accounts are verified instantly.</p>
              </div>
            </div>
            ...
            <div className="bg-[#fafaf8] border border-stone-200 rounded-2xl overflow-hidden">
              <div className="aspect-video bg-slate-900 p-4 flex flex-col">
                <p className="text-[10px] text-amber-400 font-semibold mb-2">Your Migration Manifest</p>
                <div className="flex-1 bg-slate-800 rounded-lg overflow-hidden">
                  <div className="grid grid-cols-3 bg-slate-700 px-3 py-1.5">
                    <span className="text-[8px] text-slate-400 font-medium">Category</span>
                    <span className="text-[8px] text-slate-400 font-medium text-right">Items</span>
                    <span className="text-[8px] text-slate-400 font-medium text-right">Include</span>
                  </div>
                  {[
                    { cat: 'Listings', count: '42', on: true },
                    { cat: 'Photos', count: '1,284', on: true },
                    { cat: 'Reservations', count: '318', on: true },
                    { cat: 'Guests', count: '204', on: true },
                    { cat: 'Rate Strategies', count: '12', on: true },
                    { cat: 'Automations', count: '8', on: false },
                  ].map((row) => (
                    <div key={row.cat} className="grid grid-cols-3 px-3 py-1 border-b border-slate-700">
                      <span className="text-[8px] text-slate-300">{row.cat}</span>
                      <span className="text-[8px] text-slate-300 text-right">{row.count}</span>
                      <div className="flex justify-end">
                        <div className={`w-5 h-2.5 rounded-full ${row.on ? 'bg-amber-500' : 'bg-slate-600'}`} />
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="p-5">
                <h3 className="text-lg font-bold text-slate-900 mb-1">Review Your Manifest</h3>
                <p className="text-slate-500 text-sm leading-relaxed">See exactly what data will migrate — listings, guests, reservations, photos, and more.</p>
              </div>
            </div>

            {/* Step 3 — Choose Pricing & Pay */}
            <div className="bg-[#fafaf8] border border-stone-200 rounded-2xl overflow-hidden">
              <div className="aspect-video bg-slate-900 p-4 flex flex-col gap-2">
                <p className="text-[10px] text-amber-400 font-semibold">Choose Your Plan</p>
                <div className="flex gap-2 flex-1">
                  {[
                    { name: 'Growth', price: '$349', listings: '11–50', pop: false },
                    { name: 'Professional', price: '$699', listings: '51–150', pop: true },
                    { name: 'Business', price: '$999', listings: '151–300', pop: false },
                  ].map((tier) => (
                    <div key={tier.name} className={`flex-1 rounded-lg p-2 flex flex-col ${tier.pop ? 'bg-amber-500' : 'bg-slate-800'}`}>
                      {tier.pop && <span className="text-[7px] font-bold text-slate-900 mb-1">MOST POPULAR</span>}
                      <p className={`text-[9px] font-semibold ${tier.pop ? 'text-slate-900' : 'text-slate-300'}`}>{tier.name}</p>
                      <p className={`text-[8px] mb-1 ${tier.pop ? 'text-slate-800' : 'text-slate-500'}`}>{tier.listings} listings</p>
                      <p className={`text-sm font-extrabold ${tier.pop ? 'text-slate-900' : 'text-white'}`}>{tier.price}</p>
                      <div className={`mt-auto h-4 rounded text-[7px] font-semibold flex items-center justify-center ${tier.pop ? 'bg-slate-900 text-white' : 'bg-slate-700 text-slate-300'}`}>
                        Select
                      </div>
                    </div>
                  ))}
                ...
                <div className="flex items-center gap-1.5 bg-slate-800 rounded px-2 py-1">
                  <svg className="w-3 h-3 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
                  <span className="text-[8px] text-slate-400">Powered by Stripe — PCI compliant, encrypted checkout</span>
                </div>
              </div>
              <div className="p-5">
                <h3 className="text-lg font-bold text-slate-900 mb-1">Choose Pricing &amp; Pay</h3>
                <p className="text-slate-500 text-sm leading-relaxed">Pick flat-rate or per-listing pricing, add optional extras, and pay securely via Stripe.</p>
              </div>
            </div>

            {/* Step 4 — Track Progress & Verify */}
            <div className="bg-[#fafaf8] border border-stone-200 rounded-2xl overflow-hidden">
              <div className="aspect-video bg-slate-900 p-4 flex flex-col gap-1.5">
                <div className="flex items-center justify-between mb-1">
                  <p className="text-[10px] text-amber-400 font-semibold">Migration Progress</p>
                  <span className="text-[8px] text-emerald-400 font-semibold">● Running</span>
                </div>
                {[
                  { cat: 'Listings', pct: 100, done: true },
                  { cat: 'Photos', pct: 74, done: false },
                  { cat: 'Guests', pct: 100, done: true },
                  { cat: 'Reservations', pct: 41, done: false },
                  { cat: 'Rate Strategies', pct: 100, done: true },
                  { cat: 'Automations', pct: 0, done: false },
                ].map((row) => (
                  <div key={row.cat}>
                    <div className="flex justify-between mb-0.5">
                      <span className="text-[8px] text-slate-400">{row.cat}</span>
                      <span className={`text-[8px] font-medium ${row.done ? 'text-emerald-400' : row.pct > 0 ? 'text-amber-400' : 'text-slate-600'}`}>
                        {row.done ? '✓' : row.pct > 0 ? `${row.pct}%` : '—'}
                      </span>
                    </div>
                    <div className="h-1.5 bg-slate-700 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all ${row.done ? 'bg-emerald-500' : 'bg-amber-500'}`}
                        style={{ width: `${row.pct}%` }}
                      />
                    </div>
                  </div>
